...
Definition term18 (x0 : Prop) := forall y0 : Prop, ((~ x0) -> ~ y0) = (y0 -> x0).
...
Definition term13 (x0 : Prop) := @eq Prop ((~ True) -> ~ x0).
Definition term1 (x0 : Prop) := (x0 = True) \/ (x0 = False).
Definition term14 := imp (~ False).
Definition term19 := forall y0 : Prop, forall y1 : Prop, ((~ y0) -> ~ y1) = (y1 -> y0).

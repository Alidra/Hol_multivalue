...
Definition term13 (x0 : Prop) := @eq Prop (False -> x0).
Definition term10 (x0 : Prop) := (~ False) \/ x0.
Definition term14 := or (~ False).
Definition term2 (x0 : Prop) := fun y0 : Prop => (y0 -> x0) = ((~ y0) \/ x0).
Definition term15 (x0 : Prop) := forall y0 : Prop, (x0 -> y0) = ((~ x0) \/ y0).
...
Definition term16 := forall y0 : Prop, forall y1 : Prop, (y0 -> y1) = ((~ y0) \/ y1).

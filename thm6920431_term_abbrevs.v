...
Definition term5 := @eq nat (@ε nat (fun y0 : nat => forall y1 : nat, ((Nat.add y0 y1) = y1) /\ ((Nat.add y1 y0) = y1))).
Definition term1 (a0 : Type') (x0 : type1400 a0) := @ε a0 (fun y0 : a0 => forall y1 : a0, ((x0 y0 y1) = y1) /\ ((x0 y1 y0) = y1)).
Definition term4 := @eq nat (@neutral nat Nat.add).
...
Definition term3 := @ε nat (fun y0 : nat => forall y1 : nat, ((Nat.add y0 y1) = y1) /\ ((Nat.add y1 y0) = y1)).
